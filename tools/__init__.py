...
plan 与 critic 只依赖 LLM，不依赖生图后端，所以**总是注册**。
...
def build_tools(config, plan_agent=None, critic_agent=None) -> list[Tool]:
    """按 config 装配工具集。config: AgentConfig。

    plan_agent / critic_agent 是给离线测试用的注入口：不给的话，任何跑 build_tools
    的测试都会被逼着造一个真的 OpenAI client（AgentLLM 在空 key 上直接抛 ValueError）。
    """
    if plan_agent is None:
        plan_agent = PlanAgent(name="计划", llm=AgentLLM(**config.plan_llm_kwargs))
    if critic_agent is None:
        critic_agent = CriticAgent(name="评审", llm=AgentLLM(**config.critic_llm_kwargs))

    tools: list[Tool] = [
        AddTool(),
        PlanTool(plan_agent=plan_agent),
        CriticTool(critic_agent=critic_agent),
    ]
